...
const ReceiptDetailPage = () => {
	...
	const renderSteps =
		oneReceipt?.steps?.length > 0
			? oneReceipt.steps.map((item, index) => (
					<SectionContainer key={index}>
						<>
							<TitleH2 title={item.titleOfStep} theClass={''} />
							<Paragraph text={item.text} />
							{item.infobox && item.infobox.length > 0
								? item.infobox.map((info, index) => (
										<InfoBox key={index} title={info.title} text={info.text} />
								  ))
								: null}
						</>
					</SectionContainer>
	...
	return (
		<>
			<TitleWrapper
				...
				children={
					oneReceipt.categories.data.length ? <Tag tagItems={oneReceipt.categories.data} /> : null
				}
			/>
			<FullscreenImage
				image={
			...
			<SectionContainer>
				<>
					<TitleH2 title='Zutaten' theClass='' />
					<PortionCalculation
						number={theportion}
					...
					{oneReceipt.optional_Ingredients.length > 0 ? (
						<>
							<TitleH2 title='Optionale Zutaten' theClass='' />
							<TilesRound
